...
public class QuestionsDataBaseSource implements QuestionsDataSource {

    private DBHelper mDbHelper;
    private SQLiteDatabase mSQLiteDataBase;
    private Context mContext;

    public QuestionsDataBaseSource(Context context) {
        mContext = context;
    }
    ...
    @Override
    public void addAnswer(List<AnswerToQuestion> answers, CallBackAdd callBack) {

        SQLiteDatabase dataBase = conectionDataBase();
        for (AnswerToQuestion answer : answers) {
            ContentValues contentValues = new ContentValues();
            contentValues.put(ConstantQuestion.NAME, answer.getName());
            contentValues.put(ConstantQuestion.HASHTAG, answer.getHashtag());
            contentValues.put(ConstantQuestion.ID_USER, answer.getIdUser());
            contentValues.put(ConstantQuestion.URL, answer.getUrl());
            contentValues.put(ConstantQuestion.TYPE, answer.getTypeUser());
            contentValues.put(ConstantQuestion.SELECTED, answer.getSelected() ? 1 : 0);
            dataBase.insert(ConstantQuestion.DB_TABLE_ANSWER_TO_QUESTION, null, contentValues);
        }
        callBack.success();
    }

    @Override
    public void getQuestion(int typeUser, CallBackGetQuestion callBack) {
